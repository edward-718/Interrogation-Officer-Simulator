...
/* ---------- 同义词扩展表 ---------- */
window.SYNONYMS = {
  '主谋': ['主谋', '策划', '组织者', '指使', '幕后', '老大', '带头'],
  '当晚': ['当晚', '那晚', '案发当晚', '那天晚上', '夜里'],
  '进去': ['进去过', '进过', '去过', '在场', '到过', '进入'],
  '钱': ['钱', '赃款', '现金', '现钞', '赃物', '款项'],
  '账户': ['账户', '账号', '卡号', '银行卡', '公户', '对公账户'],
  '凶器': ['凶器', '刀', '武器', '工具', '利器'],
  '同伙': ['同伙', '帮手', '一起', '参与', '伙同', '合伙'],
  '杀人': ['杀人', '故意杀人', '谋杀', ' homicide'],
  '证据': ['证据', '指纹', 'DNA', '监控', '录像', '视频'],
  '动机': ['动机', '原因', '理由', '为什么', '目的'],
  '认罪': ['认罪', '承认', '招了', '交代', '供述', '坦白'],
  '家人': ['家人', '老婆', '孩子', '女儿', '儿子', '母亲', '父亲', '爸妈'],
  '后悔': ['后悔', '悔改', '忏悔', '对不起', '错了'],
  '威胁': ['威胁', '恐吓', '威胁', '逼迫', '逼供']
};
     ...
     ===================================================== */
  {
    ...
    stages: [
      /* ---------- 阶段 1：承认主谋身份 ---------- */
      {
        ...
        crossRules: [
          { tone: 'friendly', matchKeywords: ['你好', '请问', '可以'], moodDelta: 5, reply: '（冷笑）装什么好人？有事直说。' },
          { tone: 'logical', matchKeywords: ['因为', '所以', '推理'], moodDelta: 3, reply: '（不屑）跟我讲道理？老子不吃这套。' }
        ],
        fallback: {
      ...
      /* ---------- 阶段 3：找到赃款藏匿地 ---------- */
      {
        ...
        endingLine: '（你起身准备离开，沈墨突然开口，声音轻得像叹息）……女儿三年前就死了。我只是想再听你们提一次她的名字。'
      }
    ]
  },
     ...
     ===================================================== */
  {
    ...
    stages: [
      /* ---------- 阶段 1：承认自己扮演"客服"角色 ---------- */
      {
        ...
        crossRules: [
          { tone: 'intimidate', matchKeywords: ['说', '交代', '快说'], moodDelta: 15, reply: '（崩溃大哭）不要！我说！我什么都说！求你别逼我了……' },
          { tone: 'deceive', matchKeywords: ['知道', '了解', '其实'], moodDelta: 8, reply: '（慌乱）你……你怎么知道的？谁跟你说的？' }
        ],
        fallback: {
      ...
      /* ---------- 阶段 3：找到受害者打款账户 ---------- */
      {
        ...
        endingLine: '（你合上笔录起身，林婉突然抬头，眼神里有一丝解脱）……账户里的钱，我昨天已经全部转给受害者了。你们查到的那个账户，是空的。'
      }
    ]
  },
     ...
     ===================================================== */
  {
    ...
    stages: [
      /* ---------- 阶段 1：承认当晚进入过死者家 ---------- */
      {
        ...
        crossRules: [
          { tone: 'intimidate', matchKeywords: ['快说', '老实', '别想'], moodDelta: 2, reply: '（推了推眼镜）警官，火气这么大对身体不好。' },
          { tone: 'deceive', matchKeywords: ['有人', '看到', '证人'], moodDelta: 5, reply: '（推了推眼镜）警官，你的信息来源不太可靠。' }
        ],
        fallback: {
      ...
      /* ---------- 阶段 3：套出杀人动机 ---------- */
      {
        ...
        endingLine: '（你收起笔录起身，老K忽然笑了，那笑容让房间温度降了几分）……凶器在你们局里的证物室。我十年前就交过了。'
      }
    ]
  }
];
